...
import { AddSubResponse } from '@/components/templates/ThirdPartyManagement/Edit/Response/SubResponse'
...
export const AddResponse = (prods: Prods): any => {
  ...
  //     setMappingCode(prods.response.featureAPIPropertiesId)
  //   } else {
  ...
  const addNewSubResponse = () => {
    prods.setIsDisableSubmit(false)
    const newSubParamId = (childs.length > 0) ? ((childs[childs.length - 1]?.index ?? 0) + 1) : 1;
