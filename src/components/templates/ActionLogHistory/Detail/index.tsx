...
export const DialogDetail = ({ id, refetch }: { id: number; refetch: any }) => {
  const { t } = useTranslation('actionLogHistory/detail')
  const { hideDialog } = useDialog()
  const [values] = useDetail(id, refetch)
  const { tableData, columns, dataDetail, control, showCompare, valueCompare } =
    values
