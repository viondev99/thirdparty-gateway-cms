...
import { AddSubRequest } from '@/components/templates/ApiFeatureManagement/Create/Request/SubRequest'
import {
  isMaxLength,
  isNumInteger,
  isNumeric,
...
export const AddRequest = (prods: Prods): any => {
  ...
  const changeParams = (event: any) => {
    ...
    prods.request.name = event.target.value
  }
  ...
  const canAddSubRequest = () => {
    return (
      prods.request.index && dataTypeIsArrayOrObject(prods.request.dataTypeId)
    )
  }
