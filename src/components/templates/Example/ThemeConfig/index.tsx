...
} from '@/config/themeDefaultConfig'
...
import { useQueryGetThemeConfigAPI } from '@/service/resources/theme/get'
import { Typography } from '@mui/material'
import ButtonConfig from './components/ButtonConfig'
import ColorThemeConfig from './components/ColorThemeConfig'
import FontConfig from './components/FontConfig'

export const ThemeConfig = () => {
  const { isLoading, isError, data, refetch } = useQueryGetThemeConfigAPI({
    companyId: 51,
  })
  ...
  if (data) {
    ...
    return (
      <div className='bg-[#E5E5E5] flex flex-col'>
        <div className='bg-white h-30 w-full flex px-10 items-center'>
          <Typography variant='h6' className='uppercase'>
            Cấu hình layout,theme
          </Typography>
        </div>

        <div className='m-15'>
          <ColorThemeConfig refetch={refetch} />
        </div>

        <div className='m-15'>
          <FontConfig refetch={refetch} />
        </div>

        <div className='m-15'>
          <ButtonConfig refetch={refetch} />
        </div>
      </div>
    )
  }
