...
import { useDialogDeleteClientMapping } from './useDialogDeleteClientMapping'
import { Box, Typography } from '@mui/material'
import { DialogConfirmCustom } from '@/components/organism/DialogConfirmCustom'

export const DialogDeleteClientMapping = ({
  id,
  refetch,
}: {
  id: number
  ...
  const [values, handles] = useDialogDeleteClientMapping(id, refetch)
    ...
    >
      <Box className='flex flex-col justify-center font-medium text-[20px] max-w-[351px] m-auto text-center'>
        <Typography variant='h4'>
          Are you sure you want to delete this client mapping?
        </Typography>
      </Box>
    </DialogConfirmCustom>
  )
