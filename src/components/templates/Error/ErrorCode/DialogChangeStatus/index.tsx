...
import { useDialogChangeStatusErrorCode } from './useDialogChangeStatusErrorCode'
import { DialogCustom } from '@/components/organism/DialogCustom'
import { Box, Typography } from '@mui/material'
import { ButtonCustom } from '@/components/atoms/ButtonCustom'
import { DialogConfirmCustom } from '@/components/organism/DialogConfirmCustom'

export const DialogChangeStatusErrorCode = ({
  id,
  refetch,
}: {
  id: number
  ...
  const [values, handles] = useDialogChangeStatusErrorCode(id, refetch)
    ...
    >
      <Box className='flex flex-col justify-center font-medium text-[20px] max-w-[351px] m-auto text-center'>
        <Typography variant='h4'>
          Are you sure you want to publish this error code?
        </Typography>
      </Box>
    </DialogConfirmCustom>
  )
