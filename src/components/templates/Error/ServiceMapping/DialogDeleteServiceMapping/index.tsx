...
import { useDialogDeleteServiceMapping } from './useDialogDeleteServiceMapping'
import { DialogConfirmCustom } from '@/components/organism/DialogConfirmCustom'

export const DialogDeleteServiceMapping = ({
  id,
  refetch,
}: {
  id: number
  ...
  const [values, handles] = useDialogDeleteServiceMapping(id, refetch)
    ...
    >
      <Box className='flex flex-col justify-center font-medium text-[20px] max-w-[350px] m-auto text-center'>
        <Typography variant='h4'>
          Are you sure you want to delete this service mapping?
        </Typography>
      </Box>
    </DialogConfirmCustom>
  )
